...
$url = 'https://restcountries.eu/rest/v2/name/' . $feature['properties']['name'];
...
for ($i = 0; $i < count($decoded); $i++) {
  if ($decoded[$i]['iso3'] == $feature['id'] && $decoded[$i]['population'] > 200000) {
    ...
    $j++;
  } elseif (!$feature['properties']['name']) {
...
$url ='https://api.pexels.com/v1/search?page=1&per_page=5&query=' . $feature['properties']['name'];
$url = str_replace(' ', '%20', $url);
